...
dnn::~dnn() {
    ...
    if(VdW.size()>0) {
        for(int i=0; i<n_layers; i++)
            delete Vdb[i],VdW[i],Sdb[i],SdW[i],Vdb_corrected[i],VdW_corrected[i],Sdb_corrected[i],SdW_corrected[i];
        VdW.clear();
        SdW.clear();
        Vdb.clear();
        Sdb.clear();
...
void dnn::initialize_momentum_rms() {
    VdW.push_back(NULL);
    SdW.push_back(NULL);
    Vdb.push_back(NULL);
    Sdb.push_back(NULL);
    VdW_corrected.push_back(NULL);
    SdW_corrected.push_back(NULL);
    Vdb_corrected.push_back(NULL);
    Sdb_corrected.push_back(NULL);
    for(int l=1; l<n_layers; l++) {
        ...
        VdW_corrected.push_back(new float[layer_dims[l-1]*layer_dims[l]]);
        SdW_corrected.push_back(new float[layer_dims[l-1]*layer_dims[l]]);
        Vdb_corrected.push_back(new float[layer_dims[l]]);
        Sdb_corrected.push_back(new float[layer_dims[l]]);
...
void dnn::initialize_batch_norm_weights() {
    default_random_engine rng(weights_seed);
    // random number drawn from normal distribution with  mean=0 and std=1
    normal_distribution<float> dist_norm(0,1);
    B.push_back(NULL);
    ...
    for(int l=1; l<n_layers; l++) {
        B.push_back(new float[layer_dims[l]]);
        dB.push_back(new float[layer_dims[l]]);
        G.push_back(new float[layer_dims[l]*layer_dims[l-1]]);
        dG.push_back(new float[layer_dims[l]*layer_dims[l-1]]);
        for(int j=0; j<layer_dims[l-1]*layer_dims[l]; j++)
            // scale the G to the same range to W
            // sqrt(1/(n[l]*keep_probs[l])) for "sigmoid"
            if(l==n_layers-1 || activation_types[l]=="sigmoid")
                G[l][j]=dist_norm(rng)*sqrt(1.0/(layer_dims[l-1]*keep_probs[l-1]));
        // sqrt(2/(n[l]*keep_probs[l])) for "ReLU"
            else if(activation_types[l]=="ReLU")
                G[l][j]=dist_norm(rng)*sqrt(2.0/(layer_dims[l-1]*keep_probs[l-1]));
...
void dnn::Adam_optimize(const float &initial_learning_rate,const float &beta_1,const float &beta_2,const int &num_epochs,const int &epoch_step,const int &train_step) {
	...
	memcpy(VdW_corrected[l],VdW[l],sizeof(float)*layer_dims[l]*layer_dims[l-1]);
	memcpy(SdW_corrected[l],SdW[l],sizeof(float)*layer_dims[l]*layer_dims[l-1]);
	memcpy(Vdb_corrected[l],VdW[l],sizeof(float)*layer_dims[l]);
	memcpy(Sdb_corrected[l],SdW[l],sizeof(float)*layer_dims[l]);

        // get bias corrected momentum and rms
        cblas_sscal(layer_dims[l]*layer_dims[l-1], bias_beta_1,VdW_corrected[l],1);
        cblas_sscal(layer_dims[l], bias_beta_1,Vdb_corrected[l],1);
        cblas_sscal(layer_dims[l]*layer_dims[l-1], bias_beta_2,SdW_corrected[l],1);
        cblas_sscal(layer_dims[l], bias_beta_2,Sdb_corrected[l],1);
	// SdW,Sdb=sqrt(SdW),sqrt(Sdb)
	vsSqrt(layer_dims[l]*layer_dims[l-1],SdW_corrected[l],SdW_corrected[l]);
	vsSqrt(layer_dims[l],Sdb_corrected[l],Sdb_corrected[l]);
	// add epsilon to prevent overflow
	for(int i=0;i<layer_dims[l]*layer_dims[l-1];i++)
		SdW_corrected[l][i]+=1e-8;
	for(int i=0;i<layer_dims[l];i++)
		Sdb_corrected[l][i]+=1e-8;
	// VdW,Vdb= VdW/sqrt(SdW+epsilon),Vdb/sqrt(Sdb+epsilon)
	vsDiv(layer_dims[l]*layer_dims[l-1],VdW_corrected[l],SdW_corrected[l],VdW_corrected[l]);
	vsDiv(layer_dims[l],Vdb_corrected[l],Sdb_corrected[l],Vdb_corrected[l]);

        // update the weights and bias
	cblas_saxpy(layer_dims[l]*layer_dims[l-1],-learning_rate,VdW_corrected[l],1,W[l],1);
	cblas_saxpy(layer_dims[l],-learning_rate,Vdb_corrected[l],1,b[l],1);
    }
...
void dnn::train_and_dev(const vector<float> &_X_train,const vector<int> &_Y_train,const vector<float>& _X_dev, const vector<int>& _Y_dev,const int &n_train, const int &n_dev, const int num_epochs=100,float learning_rate=0.001,const float _lambda=0,int batch_size=128,string optimizer="gradient_descent",bool batch_norm=false,bool print_cost=false) {
    ...
    if(batch_norm==true) initialize_batch_norm_weights();
    if(optimizer=="Adam") initialize_momentum_rms();
